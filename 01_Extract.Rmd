...
out_dir <- file.path("./01_data_output")
...
lanes <- st_read(file.path(shp_dir, "Tracks.gpkg"), quiet = TRUE)
lane_segs <- st_read(file.path(shp_dir, "Lane segments.gpkg"), quiet = TRUE)
...
st_drop_geometry(spec_data_mean) %>% 
  write.csv(file.path(out_dir, "mean_index_values.csv"), row.names = FALSE)
...
write.csv(file.path(out_dir, "pixel_index_values.csv"), row.names = FALSE)
